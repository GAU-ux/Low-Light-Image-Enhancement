...
def calculate_psnr(target, ref):
    ...
    if mse == 0:
        return float('inf')  # Return infinity for PSNR when MSE is zero
...
def metrics(im_dir, label_dir):
    ...
    if n > 0:  # Check if there are images processed to avoid division by zero
        avg_psnr = avg_psnr / n
        avg_ssim = avg_ssim / n
        avg_lpips = avg_lpips / n
    else:
        print("No images processed. Cannot calculate metrics.")
